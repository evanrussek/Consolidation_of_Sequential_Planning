...
%  before carrying out  second level analysis in SPM. In terms of 
%  smoothing, 12mm is a good starting point 


%% First, provide some details of the data files to be analysed
outStub         = 'SourceRecon';                                                           % Sub-folder to output beamformer data files and images to
inFiles{1}      = 'D:\Project 1\ChainRL\Subject 1\cqdceffMpPart1_11';      % List of input pre-processed SPM MEG data files (full path!)
% inFiles{2}      = 'C:\Users\dbush\Documents\Ingrid Bits\Part 4\effMpSubject4Run2.mat';
% inFiles{3}      = 'C:\Users\dbush\Documents\Ingrid Bits\Part 4\effMpSubject4Run3.mat';


%% Then, provide some parameter settings for the analysis
baseWin         = [];           % Time window for the baseline period (or leave this empty to do one time window only, seconds)
testWin         = [2 5];        % Time window for the period of interest (NOTE that baseline and test time windows must be the same length, seconds)
frqBand         = [0 300];      % Frequency band to extract integrated power values / source weights for (Hz)
...
for s   = 1
    ...
    matlabbatch{3}.spm.tools.beamforming.sources.plugin.grid.constrain = 'iskull';  % Constraint the source locaisation results to lie within the skull
    ...
    matlabbatch{4}.spm.tools.beamforming.features.woi = [min([baseWin testWin]) max([baseWin testWin])]*1000;   % Which time window to include
    ...
    matlabbatch{6}.spm.tools.beamforming.output.plugin.image_power.sametrials = false;      % This ia a bootstrapping option - set to false
    ...
    matlabbatch{6}.spm.tools.beamforming.output.plugin.image_power.result = 'singleimage';  % We would like a single image for each condition (i.e. not for each trial)
